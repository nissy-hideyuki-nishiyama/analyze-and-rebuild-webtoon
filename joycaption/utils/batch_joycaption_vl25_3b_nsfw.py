...
# from safetensors.torch import load_file  # 追加
...
class QwenVLCaptionPipeline:
    def __init__(
        self,
        model_dir: str = "/mnt/ssd-data/common/huggingface/wangkanai/qwen2.5-vl-3b-instruct",
        # weights_path: str = "/mnt/ssd-data/common/huggingface/wangkanai/qwen2.5-vl-3b-instruct/qwen2.5-vl-3b-instruct-abliterated.safetensors",
        # base_model_id: str = "Qwen/Qwen2.5-VL-3B-Instruct",
        # weights_filename: str = "qwen2.5-vl-3b-instruct-abliterated.safetensors",
        device: str = "cuda",
    ):
        self.device = device
        print(f"Loading Qwen2.5-VL on {self.device}...")
        
        # 1. 設定 (Config) と前処理 (Processor) を公式基盤モデルから取得
        # config = AutoConfig.from_pretrained(base_model_id, trust_remote_code=True)
        self.processor = AutoProcessor.from_pretrained(model_dir, trust_remote_code=True)
        
        # 2. 公式 Config と重みファイル名を指定してモデルをロード
        ...
        # 2. Configを元にモデル構造を初期化 (重みは一旦ダミー/未初期化)
        # self.model = Qwen2_5_VLForConditionalGeneration(config)

        # 3. safetensors から指定の重みファイルを直接ロードして流し込む
        # print(f"Loading weights from {weights_path}...")
        # state_dict = load_file(weights_path)
        # self.model.load_state_dict(state_dict)

        # 4. 型変換とデバイス（GPU）への配置
        # self.model = self.model.to(dtype=torch.bfloat16, device=self.device).eval()
